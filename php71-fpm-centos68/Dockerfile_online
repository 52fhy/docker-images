...
RUN wget http://pecl.php.net/get/redis-${PHPREDIS_VER}.tgz && tar xvf redis-${PHPREDIS_VER}.tgz && cd redis-${PHPREDIS_VER} && phpize  && ./configure  && make && make install && cd ../ && rm -rf redis-*

RUN wget http://pecl.php.net/get/swoole-${SWOOLE_VER}.tgz && tar xvf swoole-${SWOOLE_VER}.tgz && cd swoole-${SWOOLE_VER} && phpize  && ./configure  && make && make install && cd ../ && rm -rf swoole-*

RUN wget http://pecl.php.net/get/yar-${YAR_VER}.tgz && tar xvf yar-${YAR_VER}.tgz && cd yar-${YAR_VER} && phpize  && ./configure  && make && make install && cd ../ && rm -rf yar-*

RUN echo "extension=redis.so" >> "$PHP_INI_FILE" && \
